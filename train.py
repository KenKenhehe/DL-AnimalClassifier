...
from util_dls import train, test, MLPNet, AnimalNetCNN
...
LABELS = {"Cat": 0, "Dog": 1, "Capybara": 2, "Deer": 3,"Kangaroo": 4}
...
animal_dataset = AnimalDataset(data_file_list, transform=transform)
...
#model = AnimalNetCNN(size_x, size_y)
...
# Modify the classifier for your specific case (assuming 5 classes)
...
train(model=model, train_loader=train_loader, validation_loader= validation_loader, 
      criterion=nn.CrossEntropyLoss(), optimizer=torch.optim.SGD(model.parameters(), lr=0.001), n_epochs=70)

model.load_state_dict(torch.load('animal_model_VGG.pt'))
